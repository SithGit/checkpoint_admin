...
export class FormsAdvancedComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.eventService.broadcast(EventType.CHANGE_PAGE_TITLE, {
            ...
            breadCrumbItems: [
                { label: 'Forms', path: '.' },
                { label: 'Advanced Form', path: '.', active: true },
            ],
        });
    ...
    search1: OperatorFunction<string, readonly string[]> = (text$: Observable<string>) =>
        text$.pipe(
            ...
            map((term) => (term.length < 2 ? [] : this.statesList.filter((v) => v.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10))),
        );
    ...
    search: OperatorFunction<string, readonly string[]> = (text$: Observable<string>) => {
        ...
        return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(
            map((term) => (term === '' ? this.statesList : this.statesList.filter((v) => v.toLowerCase().indexOf(term.toLowerCase()) > -1)).slice(0, 10)),
        );
    };
}
